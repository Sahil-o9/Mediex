...
import { useLogin, useRegister } from "@/hooks/mediexApi/useAuthApi";
import { ApiError } from "@/lib/mediexApi/client";
import type { ApiUser } from "@/lib/mediexApi/types";
...
/**
 * Bridges a real backend user (from /api/auth/register or /api/auth/login)
 * into the local `Patient` shape the rest of the app (home, profile,
 * appointments, emergency) already reads from AuthContext. `fallback` fills
 * in fields the backend doesn't echo back (e.g. abhaId, language, consent
 * are UI-only and never sent to the API).
 */
function mapApiUserToPatient(user: ApiUser, fallback: Partial<Patient>): Patient {
  return {
    patientId: user._id,
    fullName: user.fullName || user.name,
    dateOfBirth: user.dateOfBirth || fallback.dateOfBirth || "",
    gender: user.gender || fallback.gender || "",
    mobileNumber: user.mobileNumber || fallback.mobileNumber || "",
    email: user.email || fallback.email,
    abhaId: fallback.abhaId,
    password: "", // never keep the real password client-side; auth is handled by the JWT
    language: fallback.language || "hi",
    consent: fallback.consent ?? true,
    createdAt: user.createdAt,
    bloodGroup: user.bloodGroup || fallback.bloodGroup,
    emergencyContact: fallback.emergencyContact,
  };
}

// Extracted helper sub-components outside the main component to prevent input focus loss
function Field({
  label,
  icon,
  children,
...
export default function PatientAuthPage() {
  ...
  const { setApiPatientSession } = useAuth();
  ...
  // Login state — phone number based authentication
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      setApiPatientSession(mapApiUserToPatient(res.user, { mobileNumber: cleanMobile }));
      navigate("/patient/home");
    } catch (err) {
      setLoginError(err instanceof ApiError ? err.message : t.auth.errLoginFailed);
    }
  }
  ...
  async function handleRegister() {
    ...
    try {
      ...
      setSuccessId(res.user._id);
    } catch (error) {
      setRegError(
        error instanceof ApiError
          ? error.status === 409
            ? t.auth.errMobileExists
            : error.message
          : t.auth.errRegisterFailed,
      );
    } finally {
      setRegLoading(false);
    }

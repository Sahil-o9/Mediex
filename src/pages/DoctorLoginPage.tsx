...
export default function DoctorLoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    if (!doctorId.trim()) {
      setError(t.doctorLogin.errDoctorId);
      return;
    }
    if (!password) {
    ...
    setTimeout(() => {
      ...
      if (!res.success) {
        setError(
          res.error === "doctor-not-found"
            ? t.doctorLogin.errDoctorId
        ...
        <main className="flex flex-1 items-center justify-center px-6 pb-12">
          ...
          >
            <div className="mb-8 text-center">
              ...
              <h1 className="text-2xl font-bold text-white md:text-3xl">{t.doctorLogin.title}</h1>
              <p className="mt-1 text-slate-400">{t.doctorLogin.subtitle}</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-semibold text-slate-300">
                  {t.doctorLogin.doctorId}
                </label>
                <div className="flex items-center gap-3 rounded-2xl border-2 border-slate-700 bg-slate-850 px-4 py-3.5 transition-colors focus-within:border-medical-500 focus-within:ring-2 focus-within:ring-medical-500/20">
                  ...
                  <input
                    type="text"
                    placeholder={t.doctorLogin.doctorIdPh}
                    value={doctorId}
            ...
            <div className="mt-6 rounded-2xl bg-slate-850 p-4 text-center text-sm">
              <p className="font-semibold text-slate-400">{t.doctorLogin.demoCredentials}</p>
              <p className="mt-1 text-slate-500">
                {t.doctorLogin.doctorId}:{" "}
                <span className="font-mono font-semibold text-medical-400">doctor001</span>
              </p>
              <p className="text-slate-500">
                {t.auth.password}:{" "}
                <span className="font-mono font-semibold text-medical-400">Doctor@123</span>
              </p>
            </div>
          </motion.div>
        </main>
